...
def _save_synthetic_data(root_dir, dataset_name, resolution, n_samples, h, w):
    data = {
        "x": torch.randn(n_samples, h, w),
        "y": torch.randn(n_samples, h, w)
    }
...
def synthetic_data_dir(tmp_path):
    _save_synthetic_data(tmp_path, "darcy", SOURCE_RES, N_TOTAL, H, W)
    return tmp_path
...
class TestPTDatasetIsAbstract:

    def test_cannot_instantiate_directly(self):
        with pytest.raises(TypeError):
            PTDataset(  # type: ignore[abstract]
                root_dir=".",
                dataset_name="x",
...
class TestVertexStride:
    ...
    def test_421_to_11(self):
        assert vertex_stride(421, 11) == 42

    def test_421_to_61(self):
        assert vertex_stride(421, 61) == 7

    def test_421_to_211(self):
        assert vertex_stride(421, 211) == 2

    def test_61_to_11(self):
        assert vertex_stride(61, 11) == 6

    def test_invalid_raises(self):
        with pytest.raises(ValueError, match="Cannot stride-subsample"):
            vertex_stride(421, 10)
...
class TestStrideSubsampling:
    ...
    def test_stride_selects_correct_vertices(self, synthetic_data_dir):
        ...
        raw = torch.load(synthetic_data_dir / f"darcy_train_{SOURCE_RES}.pt")
    ...
    def test_multiple_test_resolutions(self, synthetic_data_dir):
        ...
        assert ds.test_dbs[TRAIN_RES][0]["x"].shape == (1, TRAIN_RES,
                                                        TRAIN_RES)
...
class TestEncoding:
    ...
    def test_encode_input_false_leaves_in_normalizer_none(
    ...
    def test_encode_output_false_leaves_out_normalizer_none(
            self, synthetic_data_dir):
        ds = DarcyDataset(
            root_dir=synthetic_data_dir,
            n_train=N_TRAIN,
    ...
    def test_channel_wise_normalizer_reduces_all_dims_except_channel(
            self, dataset):
        normalizer = dataset.data_processor.in_normalizer
        assert normalizer.dim == [0, 2, 3]

    def test_pixel_wise_normalizer_reduces_only_batch_dim(
            self, synthetic_data_dir):
        ds = DarcyDataset(
            root_dir=synthetic_data_dir,
            n_train=N_TRAIN,
