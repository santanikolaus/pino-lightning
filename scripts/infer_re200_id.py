"""
Cross-Re inference sweep using the frozen RE=200 pretrained FNO checkpoint.

Runs on test trajectories (indices 260-299, n=40) for each Re in the sweep list.
Equation loss (pde_loss) uses ν=1/200 (training Re) for ALL datasets — realistic
OOD detector: incoming Re is unknown, residual measures "satisfies Re=200 physics?"

RE=200 plausibility gates (from training logs, epoch 149, run 4em1mfrx):
  data_l2  TIGHT: expect ≈ val_l2=0.3998     (>10% gap = pipeline error)
...
Run from project root:
    python scripts/infer_re200_id.py --ckpt /path/to/best.ckpt
    python scripts/infer_re200_id.py --ckpt /path/to/best.ckpt --re 100,200,300,500,1000
"""
...
# Full model config: fno_kf.yaml base merged with kf_pino_re200_pretrain.yaml overrides
MODEL_CFG = {
    "model_arch": "fno",
    "data_channels": 4,
    "out_channels": 1,
...
# Re=200 plausibility anchors (training logs, epoch 149, run 4em1mfrx)
RE200_ANCHORS = {
    "data_l2": ("val_l2", 0.3998),
}
...
def parse_args():
    ...
    p.add_argument("--ckpt", required=True,
                   help="Path to best.ckpt (RE=200 pretrained Lightning checkpoint)")
    ...
    p.add_argument("--out",  default="scripts/outputs/infer_re_sweep_fixednu_re200.npz")
...
def run_re(re: int, model, device: torch.device) -> dict[str, np.ndarray]:
    ...
    print(f"  Loss : KFLoss(re=200, ν=1/200)  [fixed training Re — true OOD detector]")
    print(f"{'='*65}")

    if not path.exists():
        raise FileNotFoundError(f"Data file not found: {path}")

    # ν fixed to training Re=200 for all evaluations — realistic OOD detector:
    # incoming Re is unknown; residual measures "satisfies Re=200 physics?"
    loss_fn = KFLoss(re=200, t_interval=1.0,
...
def main():
    ...
    for re, arrs in all_results.items():
        ...
        anchor_str = ""
        if re == 200:
            d_anchor = RE200_ANCHORS["data_l2"][1]
    ...
    if 200 in all_results:
        print(f"\n  Re=200 anchor: data_l2=0.3998  (val logs, epoch 149, run 4em1mfrx)")
