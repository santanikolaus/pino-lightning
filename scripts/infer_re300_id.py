"""
Cross-Re inference sweep using the frozen RE=300 pretrained FNO checkpoint.

Runs on test trajectories (indices 260-299, n=40) for each Re in the sweep list.
Equation loss (pde_loss) uses ν=1/300 (training Re) for ALL datasets — realistic
OOD detector: incoming Re is unknown, residual measures "satisfies Re=300 physics?"

RE=300 plausibility gates (fill in from training logs after first run):
...
Run from project root:
    python scripts/infer_re300_id.py --ckpt /path/to/best.ckpt
    python scripts/infer_re300_id.py --ckpt /path/to/best.ckpt --re 100,200,300,500,1000
"""
...
# Full model config: fno_kf.yaml base merged with kf_pino_re300_pretrain.yaml overrides
MODEL_CFG = {
    "model_arch": "fno",
    "data_channels": 4,
    "out_channels": 1,
...
# Re=300 plausibility anchors — update with observed val_l2 after first run
RE300_ANCHORS = {
    "data_l2": ("val_l2", None),   # fill in from training logs
}
...
def parse_args():
    ...
    p.add_argument("--ckpt", required=True,
                   help="Path to best.ckpt (RE=300 pretrained Lightning checkpoint)")
    ...
    p.add_argument("--out",  default="scripts/outputs/infer_re_sweep_fixednu_re300.npz")
...
def run_re(re: int, model, device: torch.device) -> dict[str, np.ndarray]:
    ...
    print(f"  Loss : KFLoss(re=300, ν=1/300)  [fixed training Re — true OOD detector]")
    print(f"{'='*65}")

    if not path.exists():
        raise FileNotFoundError(f"Data file not found: {path}")

    # ν fixed to training Re=300 for all evaluations — realistic OOD detector:
    # incoming Re is unknown; residual measures "satisfies Re=300 physics?"
    loss_fn = KFLoss(re=300, t_interval=1.0,
...
def main():
    ...
    for re, arrs in all_results.items():
        ...
        anchor_str = ""
        if re == 300:
            anchor = RE300_ANCHORS["data_l2"][1]
    ...
    if 300 in all_results:
        anchor = RE300_ANCHORS["data_l2"][1]
        if anchor is None:
            print(f"\n  Re=300 anchor not set — record observed data_l2 above in RE300_ANCHORS")
