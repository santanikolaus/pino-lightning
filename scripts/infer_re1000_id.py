"""
Cross-Re inference sweep using the frozen RE=1000 pretrained FNO checkpoint.

Runs on test trajectories (indices 260-299, n=40) for each Re in the sweep list.
Equation loss (pde_loss) uses ν=1/1000 (training Re) for ALL datasets — realistic
OOD detector: incoming Re is unknown, residual measures "satisfies Re=1000 physics?"

Re=1000 data: NS_fine_Re1000_T128_indep.npy (independent ICs, tau_corr=80, N_eff=480).

RE=1000 plausibility gates (fill in from training logs after first run):
...
Run from project root:
    python scripts/infer_re1000_id.py --ckpt /path/to/best.ckpt
    python scripts/infer_re1000_id.py --ckpt /path/to/best.ckpt --re 100,200,300,500,1000
"""
...
def parse_args():
    ...
    p.add_argument("--ckpt", required=True,
                   help="Path to best.ckpt (RE=1000 pretrained Lightning checkpoint)")
    ...
    p.add_argument("--out",  default="scripts/outputs/infer_re_sweep_fixednu_re1000.npz")
...
def run_re(re: int, model, device: torch.device) -> dict[str, np.ndarray]:
    ...
    print(f"  Loss : KFLoss(re=1000, ν=1/1000)  [fixed training Re — true OOD detector]")
    print(f"{'='*65}")

    if not path.exists():
        raise FileNotFoundError(f"Data file not found: {path}")

    loss_fn = KFLoss(re=1000, t_interval=1.0,
...
def main():
    ...
    for re, arrs in all_results.items():
        ...
        anchor_str = ""
        if re == 1000:
            anchor = RE1000_ANCHORS["data_l2"][1]
    ...
    if 1000 in all_results:
        anchor = RE1000_ANCHORS["data_l2"][1]
        if anchor is None:
            print(f"\n  Re=1000 anchor not set — record observed data_l2 above in RE1000_ANCHORS")
